...
# This takes a long time to run.. open to suggestions of better ways
party <- array(0, dim=c(0, nrow(df)))
i = 1
while (TRUE) {
  print(i)
  if (df[i,]$presvote16post == 1 & df[i,]$house3 == 1) {
    party[i] <- "Democrat"
  } else if (df[i,]$presvote16post == 2 & df[i,]$house3 == 2){
    party[i] = "Republican"
  } else {
    party[i] = "Swinger"
  }
